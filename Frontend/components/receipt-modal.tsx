...
export default function ReceiptModal({
  ...
  useEffect(() => {
    if (isOpen && receiptData) {
      const processItems = async () => {
        // Proses data receipt untuk memastikan semua item memiliki path gambar yang valid
        const itemsWithValidImages = await Promise.all(
          receiptData.items.map(async (item: any) => {
            // Jika item sudah memiliki gambar, coba validasi
            if (item.image) {
            ...
            // Jika tidak ada gambar yang valid, coba gunakan path berdasarkan ID atau nama
        ...
        console.log("Receipt data items:", processedReceiptData.items);
        processedReceiptData.items.forEach((item: any, index: number) => {
  ...
  // Fungsi untuk mendapatkan nama file gambar dari path
